...
export const AnimationGrid = ({ cards }: { cards: Card[] }) => {
  ...
  return (
    <div className='relative mx-auto grid h-[500px] w-full max-w-7xl grid-cols-1 gap-4 p-10 md:grid-cols-3'>
      {cards.map((card, i) => (
            ...
            className={cn(
              ...
              selected?.id === card.id
                ...
                : lastSelected?.id === card.id
                  ? 'z-40 h-full rounded-xl bg-white'
...
const BlurImage = ({ card }: { card: Card }) => {
  ...
  return (
    <Image
      ...
      className={cn(
        'absolute inset-0 h-full w-full cursor-pointer object-cover blur-md transition duration-200',
        {
          'blur-none': loaded,
        },
      )}
